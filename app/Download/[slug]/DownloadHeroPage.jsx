...
export default function DownloadHeroPage({ slug }) {
  ...
  useEffect(() => {
    let config = {
      ...
      url: `https://localhost:44367/api/Files/GetInfo/${slug}`,
    };
    ...
    axios
      ...
      .then((response) => {
        ...
        setFileData(response.data.payload);
        setIsDataLoad(true);
      })
      .catch((error) => {
        console.log(error);
  ...
  const downloadFile = async () => {
    ...
    const config = {
      method: "get",
      url: ` https://localhost:44367/api/Files/Download/${slug}`,
    ...
    try {
      ...
      // Create a blob URL and link element
      ...
      const fileName =
        FileData.files?.length > 1 ? "Files.zip" : FileData.title;
      link.setAttribute("download", fileName); // Change 'yourfile.ext' to the file's actual name and extension
        ...
        <div className="container-xs mb-2 flex items-center justify-center md:flex-col md:px-5">
          <div className=" flex w-[34%] flex-col self-end md:w-full md:self-auto">
            {IsDataLoad && (
              ...
              >
                Your {FileData.files.length > 1 ? "Files" : "File"}:{" "}
                {FileData?.title}
              </Heading>
            )}
            <Heading
            ...
            >
              Click the button below to download your file instantly.
            </Heading>
            ...
            Download Porta for desktop  
              ...
              {IsDataLoad && (
                ...
                >
                  Download {FileData.files.length > 1 ? "Files" : "File"}
                </button>
              )}
             {IsDownloadingStart && <div className="pt-2">
                <Loader />
              </div>}
            </div>
          </div>
